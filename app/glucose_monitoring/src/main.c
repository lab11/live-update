...
void main(void) {
  ...
  int beginning_index = length - 1, prev_bg = 0, current_bg = 0, expected_bg = 0, existing_basal = SCHEDULED_BASAL;
  ...
  for (int i = 1; i < 11; i++) {
    ...
    expected_bg = current_bg - ISF * calculate_net_iob(insulin_intake, length, beginning_index);
    existing_basal = calculate_next_temp(current_bg, current_bg - prev_bg, existing_basal, expected_bg);
