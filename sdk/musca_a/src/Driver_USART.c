...
static int32_t ARM_USARTx_Initialize(UARTx_Resources* uart_dev)
{
    ...
    uart_pl011_enable_intr(uart_dev->dev, UART_PL011_RX_INTR_MASK);
    uart_pl011_set_rx_fifo_lvl(uart_dev->dev, UART_PL011_RX_FIFO_LVL_1_8);
                                  ...
                                  void *data, uint32_t num)
{
    ...
    uart_dev->rx_buf = p_data;
    /* Resets previous RX counter */
    uart_dev->rx_nbr_bytes = 0;
    uart_dev->rx_req_bytes = num;

    /* Read characters already in FIFO, if any */
    while (uart_pl011_is_readable(uart_dev->dev)){
        (void)uart_pl011_read(uart_dev->dev, uart_dev->rx_buf);
        uart_dev->rx_nbr_bytes++;
        uart_dev->rx_buf++;

        if (uart_dev->rx_nbr_bytes == uart_dev->rx_req_bytes) {
...
void UARTRX1_Handler(void) {
    while (uart_pl011_is_readable(USART1_DEV.dev)){
        (void)uart_pl011_read(USART1_DEV.dev, USART1_DEV.rx_buf);
        USART1_DEV.rx_nbr_bytes++;
        USART1_DEV.rx_buf++;

        if (USART1_DEV.rx_nbr_bytes == USART1_DEV.rx_req_bytes) {
            USART1_DEV.cb_event(ARM_USART_EVENT_RECEIVE_COMPLETE);
            break;
        }
    };
}
